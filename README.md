...
Written in Python. Dependencies are installed automatically with `uv sync`. External tools (Radare2, Ghidra, ilspycmd) are optional and enable decompilation features when present.
...
# With LLM-powered analyst report (Ollama on a remote host)
...
All Python dependencies are included in the base installation — `uv sync` installs everything needed. No extra flags required.
...
| `--debug` | Write the LLM prompt to `<filename>_llm_prompt.md` for inspection. Works standalone (no `--llm-report` needed) — useful for verifying prompt content without running Ollama |
