...
class StringPattern:
    ...
    def __iter__(self):
        # Allows legacy tuple unpacking: pattern, category = sp
        return iter((self.pattern, self.category))
...
SUSPICIOUS_STRING_PATTERNS = [
    ...
    # ══════════════════════════════════════════════════════════════════
    # gMSA attacks (GoldenGMSA, etc.)
    StringPattern(r'GmsaPassword|ManagedPasswordId|msDS-ManagedPassword', "gmsa_attack", 10),
    ...
    StringPattern(r'NTLM|NtlmHash|DCC2|mscash', "ntlm_hash", 6),
    # LDAP reconnaissance
    StringPattern(r'DirectorySearcher|SearchResultCollection|DirectoryEntry', "ldap_query", 6),
    StringPattern(r'LDAP://|GC://|ldap_bind|ldap_search', "ldap_connection", 6),
    StringPattern(r'samAccountName|userPrincipalName|servicePrincipalName', "ad_attribute", 8),
    StringPattern(r'Domain Admins|Enterprise Admins|Account Operators', "ad_privileged_group", 8),
    # DCSync / replication
    StringPattern(r'DsGetNCChanges|GetNCChanges|DRSUAPI|DRSReplicaSync', "dcsync_indicator", 10),
    # .NET offensive tool indicators
    StringPattern(r'Costura\.AssemblyLoader|ProcessedByFody', "costura_fody", 6),
    StringPattern(r'ReadFromEmbeddedResources|ReadExistingAssembly', "embedded_assembly_load", 6),
    # Code-signing infrastructure (certificate OCSP/CRL)
    StringPattern(r'ocsp\.globalsign|ocsp\.digicert|ocsp\.sectigo|ocsp\.comodoca|ocsp\.entrust',
                  "codesign_ocsp", 2),
