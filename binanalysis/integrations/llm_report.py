...
def _build_prompt(results: dict, filepath: Path) -> str:
    ...
    # Version info
    vi = fmt.get("version_info", {})
    if isinstance(vi, dict) and vi.get("string_file_info"):
        sections.append(f"## Version Info\n{json.dumps(vi['string_file_info'], default=str, indent=2)}")

    # Overlay
    ov = fmt.get("overlay", {})
    if isinstance(ov, dict) and ov.get("has_overlay"):
    ...
    # Strings — categories and counts with samples
    str_data = generic.get("strings", {})
    if str_data:
        str_summary = {}
        for category, findings in str_data.items():
...
def generate_llm_report(
    ...
    If dry_run is True, only write the debug prompt file (no LLM call).
    """
